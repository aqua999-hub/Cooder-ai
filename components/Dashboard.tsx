...
// Fixed: Added 'Code' to the imports from lucide-react
import { FileCode, Clock, Zap, Activity, HardDrive, Code } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ files, logs }) => {
  ...
  const stats = [
    ...
    { label: 'AI Tasks', value: agentTasks, icon: Zap, color: 'text-yellow-400' },
    { label: 'Project Size', value: `${(totalChars / 1024).toFixed(1)} KB`, icon: HardDrive, color: 'text-purple-400' },
  ];

  return (
    <div className="h-full w-full p-8 overflow-y-auto custom-scrollbar bg-[#0d0d0d]">
      <div className="max-w-5xl mx-auto">
        <header className="mb-10">
          <h2 className="text-3xl font-bold mb-2">My Project</h2>
          <p className="text-gray-600 text-sm">A quick look at your files and AI activity.</p>
        </header>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {stats.map((stat, i) => (
            <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-3xl hover:border-[#10a37f]/30 transition-all">
            ...
            <div className="space-y-4">
              {logs.slice(-5).reverse().map((log) => (
                <div key={log.id} className="flex gap-4">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 line-clamp-2">{log.msg}</p>
                    <span className="text-[10px] text-gray-700">{new Date(log.timestamp).toLocaleTimeString()}</span>
                  </div>
                </div>
              ))}
