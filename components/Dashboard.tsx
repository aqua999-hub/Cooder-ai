    ...
    <div className="h-full w-full p-8 overflow-y-auto custom-scrollbar bg-[var(--bg-main)]">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
          {stats.map((stat, i) => (
            <div key={i} className="bg-[var(--bg-hover)] border border-[var(--border)] p-6 rounded-xl hover:border-indigo-500/30 transition-all group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[var(--bg-hover)] border border-[var(--border)] p-6 rounded-xl">
            ...
            <div className="space-y-4">
              {logs.slice(-5).reverse().map((log) => (
                <div key={log.id} className="flex gap-4">
                  <div className={`w-1 shrink-0 rounded-full ${log.role === 'agent' ? 'bg-indigo-500' : 'bg-[var(--border)]'}`} />
                  <div>
                ...
                return (
                  <div key={lang}>
                    <div className="flex justify-between text-[11px] mb-1">
                      <span className="uppercase font-bold text-[var(--text-dim)]">{lang}</span>
